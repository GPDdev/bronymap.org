...
一个隐私优先的小马迷地图。浏览器会先把选点压缩成近似恒定地面边长的 10 km 或 25 km 网格，服务器不会收到或保存原始坐标。
...
- 不读取浏览器定位权限，用户只能主动点选。
- 后端只接受网格编号，不接受经纬度。
- 新版网格按纬度分带，并用该纬带中心的余弦修正经度方向，避免 Web Mercator 网格在高纬度显著缩小。
- 每个标记都会公开昵称、联系方式和经过网格化、偏移后的大致位置。
- 升级前创建的标记保持匿名聚合，不会被追溯公开。
